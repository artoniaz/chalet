...
class _AddChaletState extends State<AddChalet> {
  ...
  void handleQualityRatingUpdate(double rating) =>
      setState(() => _chalet.quality = rating);
  void handleCleanRatingUpdate(double rating) =>
      setState(() => _chalet.clean = rating);
  void handlePaperRatingUpdate(double rating) =>
      setState(() => _chalet.paper = rating);
  void handlePrivacyRatingUpdate(double rating) =>
      setState(() => _chalet.privacy = rating);

  void handleImagesChange(List<File> imgs) => setState(() => images = imgs);
  ...
  Future<void> addImagesToStorage(String id)async {
    Reference storageRef = FirebaseStorage.instance.ref();
    TaskSnapshot addImg = await storageRef.child(id).putFile(images.first);
  ...
  Future<void> createChalet() async {
    ...
    if (_formKey.currentState!.validate()) {
      if (_chalet.quality > 0 &&
          _chalet.clean > 0 &&
          _chalet.paper > 0 &&
          _chalet.privacy > 0) {
        //TODO: dodać easy loading
        ...
        try {
          ...
          await addImagesToStorage(chaletId ?? '');
          EasyLoading.dismiss();
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      TextFormField(
                        decoration:
                            textInputDecoration.copyWith(hintText: 'Nazwa'),
                        validator: (val) =>
                            val!.isEmpty ? 'To pole jest obowiązkowe' : null,
                      ...
                      RatingBarRow(
                          label: 'Papier',
                          handleRatingUpdate: handlePaperRatingUpdate),
                      RatingBarRow(
                        label: 'Prywatność',
                      ...
                      TextFormField(
                        decoration:
                            textInputDecoration.copyWith(hintText: 'Opis'),
                        onChanged: (val) =>
                            setState(() => _chalet.description = val),
                      ),
                      VerticalSizedBox8(),
